...
整体采用黑色沉浸舞台、蓝色 AI 主色和玻璃质感卡片，营造正式发布会里的产品 Demo 区域。画面中心是手机样机，右侧是演示流程和可点击话术，页面下方保留组件库，方便直接截图进入汇报材料。
...
- 修改产品名称时，同步更新 `<title>`、Header、Hero 中的品牌文案。
- 修改配色时，优先调整 CSS `:root` 变量，避免逐个改组件颜色。
- 修改演示路径时，先更新示例话术和 `SCENES` 配置，再更新右侧流程说明。
- 增减页面模块时，保持 Hero、Live Demo、Features 三类核心区域仍可识别。
